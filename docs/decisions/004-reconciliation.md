...
라인을 SUM하면 2.5~3.0배 과대계상된다(아래 전수 실측). 003의 요구 6은 002의 I4/I5로 대체된다.

> **근거 정정:** 002는 "sessionId 단위 제거는 cross-machine resume 시 데이터 손실"을 이유로 들었으나,
> 전수 실측 결과 두 소스의 `sessionId` 교집합도 0이라 오늘 데이터에서 두 키는 같은 답을 낸다.
> `message.id` 의 진짜 정당성은 **소스 내부의 라인 grain** 이다. 아래 "전수 실측" 절 결론 2.
...
## C5. 토큰 갱신 vs 완료기준 1 (**해소됨 - 사용자가 (A)+(C) 승인, 2026-07-09**)

> **결정:** (A) ride-along 으로 시작하고 (C) 를 조사한다.
> ROADMAP 반영 완료 - 완료 기준 1의 분모 재정의, M2를 "자동 갱신" -> "ride-along" 으로 교체,
> (C) 조사를 `T4` 로 신설. `auth_skip` 은 결손율에서 빼되 **별도 지표로 반드시 노출**한다
> (숨기면 `CLAUDE.md` 6항의 정신 - "대시보드가 거짓말하지 않는다" - 에 위배).
...
## 전수 실측 (2026-07-09, 반증 조건 검증)

002의 실측은 120~150 파일 샘플이었다. 설계 확정 전에 **전수**로 다시 쟀다.
스크립트는 통계만 출력하고 프롬프트 원문·프로젝트명을 읽지 않는다.

| 측정 | WSL (2,070 파일) | Windows (52 파일) |
|---|---|---|
| usage 라인 | 28,493 | 8,569 |
| distinct `message.id` | 12,294 | 3,388 |
| **`message.id` 없는 usage 라인** | **0** | **0** |
| `timestamp`/`model` 없는 usage 라인 | 0 / 0 | 0 / 0 |
| 다중 라인 메시지 | 9,735 | 2,429 |
| 종단 usage == max (output) | **9,735/9,735 (100%)** | **2,429/2,429 (100%)** |
| output 단조 비감소 | 100% | 100% |
| **output 토큰: 라인 단순합 / 종단합** | **2.538배** | **3.004배** |
| `speed` 필드 부재 라인 | 4,053 | 1,404 |
| sidechain usage 라인 | 5,594 | 1,781 |

소스 간 교집합: **`message.id` 0건, `sessionId` 도 0건.** union = 15,684.

**결론 1 (반증 조건 통과):** `message.id` 는 모든 usage 라인에 존재한다. dedup 키로 안전하다.
종단 usage = max 라는 가정도 전수 100%다. `usage_event` grain = `message.id` 확정 (I4).

**결론 2 (002의 근거 정정):** 002는 `sessionId` 대신 `message.id` 를 고른 이유로
"cross-machine resume 시 sessionId 단위 제거는 데이터 손실"을 들었다. **오늘 데이터에서
`sessionId` 교집합도 0이므로 두 키는 같은 답을 낸다.** `message.id` 를 쓰는 진짜 이유는
크로스 소스 중복 제거가 아니라 **소스 내부의 라인 grain 붕괴**(2.5~3.0배 과대계상)다.
결론은 유지, 근거를 교체한다. 크로스 소스 dedup은 현재 no-op이며, 그래도 키를 갖추는 것은
장래 멀티머신 대비의 보험이다(비용 0).

**결론 3 (스키마 함의):** `speed` 는 usage 라인의 약 14~16%에서 **부재**한다(구버전 기록).
컬럼은 nullable이어야 하고, 단가 조회는 `speed IS NULL` 을 `standard` 로 해석해야 한다.
C4의 "필드 보존" 판단이 옳았음을 지지한다.

**결론 4 (실행 형상):** PID 1 = `init(rocky9.3)`, `systemctl is-system-running` = `offline`
(바이너리는 존재하나 systemd가 시스템을 관리하지 않음). `crond` 프로세스 1개 상시 구동.
002 축 1(cron)의 전제가 성립한다.

**미검증으로 남는 것:** `/fast` 를 켰을 때 `speed` 값이 실제로 바뀌는지. 관측된 값은
`standard` 와 부재뿐이다. 토글은 사용자 행위라 확인하지 못했다. C4의 "보존은 지금, 단가는 나중"
방침이 이 미검증을 안전하게 흡수한다.
