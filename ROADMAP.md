...
4. **소스 어댑터 확장성** - 코드 수정 없이 **설정만으로** Windows 소스를 추가해
   ...
   - 주의: dedup 키는 `sessionId` 가 아니다. usage 라인 21,924건에 distinct `message.id` 는
     9,223건(2.4:1)이고, 라인을 합치면 약 2.4배 과대계상된다. 근거는 `docs/decisions/004-reconciliation.md` C2.
...
- [ ] **M2 토큰 갱신** - accessToken 만료(약 8시간) 시 refreshToken 으로 자동 갱신.
  토큰은 로그/에러/커밋 어디에도 남기지 않는다 (`CLAUDE.md` 2항).
  - 검증: `expiresAt` 을 과거로 조작한 상태에서 수집기를 돌려 갱신 후 200을 받는다.
    로그 전문에 `grep -c "sk-\|ey[A-Za-z0-9]\{20,\}"` 가 0.
  - 의존: M1

- [ ] **M3 트랜스크립트 아카이브** - 원본 JSONL을 소멸 전에 확보한다.
  파싱보다 **보존이 먼저**다. 스키마를 몰라도 일단 복사한다.
  - 검증: 아카이브 파일 수 = 원본 파일 수. `sha256sum` 샘플 10개 일치.
  - 의존: 없음 (M1과 병렬 가능)

- [ ] **M4 cron 배선** - `claude-config/runtime/cron/` 패턴 재사용해 M1+M3를 스케줄링.
  실패 시 조용히 죽지 않도록 알림.
...
- [ ] **M5 트랜스크립트 파서 + 귀속 스키마** - `message.usage` 를 이벤트 테이블로.
...
- [ ] **M6 소스 어댑터** - WSL 어댑터 + Windows 어댑터. 중복 `message.id` 제거.
...
- [ ] **M9 Next.js 셸 + 디자인 시스템** - shadcn/ui + Tailwind v4.
