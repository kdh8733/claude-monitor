...
2. **리셋 경계 재현** [x] (2026-07-10 실측) - 리셋 시각 전후로 소진율 톱니가 관측된다.
   `resetEvents()` 로 검증. 관측 구간에서 7건의 하락(>=5%p)을 확인했다.
   - **성립하되 문구를 정정한다.** 최초 기준은 "`resets_at` 시점에 하락"이었으나, 7건 중 2건은
     `resets_at` 이 예고하지 않은 전면 리셋(2026-07-09 18:00)이었다. 정확한 기준은
     "리셋 시각 전후로 하락이 관측된다" + "예고 없는 리셋도 존재한다(대시보드에 표시)".
   - 검증: `resetEvents` 가 예고된 리셋(5건)과 예고 없는 리셋(2건)을 구분한다는 회귀 테스트.
     실측 재현: session 3건(전부 예고O), weekly_all/weekly_scoped 각 2건(예고 1 + 예고 없음 1).
...
- **`resets_at` 이 실제 리셋 시각을 예고하지 않는 사례를 관측했다 (2026-07-09).**
  `17:50:01` 에 `five_hour` 만 75% -> 0% (5시간 창의 자기 경계. 정상).
  `18:00:01` 에 **모든 버킷이 동시에 0** 이 되고 `seven_day.resets_at` 이 `null` 이 됐다.
  그 직전 `weekly_all=49%`, `weekly_scoped=76%` 였다.
  그런데 이후 `resets_at` 은 **다시 2026-07-10T12:59Z** 를 가리킨다. 즉 18:00 의 전면 리셋은
  예고된 주간 리셋이 **아니었다.** 정체 불명이다 (Anthropic 측 조작? 계정 이벤트?).
  - 완료 기준 2 는 "`resets_at` 시점에 하락한다"고 단언한다. 이 관측은 **하락이 `resets_at`
    이 아닌 곳에서도 일어남**을 보인다. 기준을 "리셋 시각 전후로 하락이 관측된다"로
    바꿔야 할 수 있다. 예고된 주간 리셋(2026-07-10T12:59Z)을 실제로 관측한 뒤 확정한다.
  - 대시보드 함의: 리셋 직후에는 "버려진 헤드룸" 구간 평균이 과대평가된다.
    관측 구간이 리셋을 걸치면 그 사실을 화면에 표시하는 편이 정직하다.
    (2026-07-11 구현 - 개요 탭 헤드룸 패널 주석 + 트렌드 리셋 마커 + 수집 상태 탭 리셋 표.
    예고/예고 없음을 구분해 표시한다.)
...
- [x] **T2 리셋 경계 재현** (2026-07-10, **완료 기준 2**) - 톱니 7건 관측.
  예고된 주간 리셋(2026-07-10 12:59Z)을 폴링 간격 안에서 정확히 포착했다.
  반례도 관측: 2026-07-09 18:00 의 예고 없는 전면 리셋. `resetEvents()` + 회귀 테스트로 못박음.
...
- [x] **배포 자동화 (CI -> Vercel)** (2026-07-12) - main push 마다 verify 통과 후 자동 배포.
  배포 직전 카나리를 한 번 더 돌려 실데이터가 섞인 빌드는 공개되지 않게 한다.
  토큰은 VERCEL_TOKEN 환경변수로 넘긴다(프로세스 목록 노출 방지).
  - 검증: deploy 잡 success, 프로덕션 URL(lang=ko)이 최신 빌드 서빙, 응답에 실데이터 0.
  - 삽질 기록: 첫 등록 토큰이 셸 확장으로 오염됨(60자 -> 64자 + 물결 2개).
    값 미노출 진단 스텝(length/has_tilde/first3/last3)으로 원인 특정. 작은따옴표 재등록으로 해결.

- [x] **보존 정책 6개월** (2026-07-10) - 사용자 결정. 주 1회 cron.
  아카이브는 원본 mtime, DB 행은 자기 시각으로 자른다. 지운 내역은 `collector_run(kind='prune')`.
  - 검증: 실데이터 0건 삭제(데이터가 아직 2개월치), 실 DB 사본에 오래된 행을 심어 삭제 경로 검증.
  - 구현 중 잡은 것: `collector_run.snapshot_id` FK 때문에 오래된 스냅샷 삭제가 터진다.
    run 행은 결손의 권위 있는 출처라 지우지 않고 포인터만 끊는다.

- [x] **CI + 카나리** (2026-07-10) - push/PR 마다 테스트 105, tsc(root+web), 데모 빌드, 카나리.
  완료 기준 6 을 사람이 아니라 기계가 지킨다. 수집기 런타임 의존성 0 도 CI 가 강제한다.
  - 검증: 카나리 자체를 변이 테스트 5종으로 검증(전부 물었다). GitHub Actions green.
